...
1. Wykorzystując wiedzę z tematu `Obsługa formularzy`. Stworzyć dwa pola do wprowadzania danych. Jedno pole do wprowadzania wartości tekstowej a drugie do wprowadzania wartości liczbowej. Dla każdego pola wstawić oddzielny przycisk do wysłania. Po wysłaniu formularza metodą `POST` zapisać podaną wartość w tablicy a następnie wyświetlić zawartość tablicy. Dodać również przycisk do usuwania pozycji z tablicy wypełnionej słowami oraz przycisk do usuwania wartości z tablicy wypełnionej liczbami.  